 ...
 * chrome.storage.local key, and the travel-arrival alarm (a stale one referencing
 * now-deleted state could otherwise misfire). Callable directly from the popup:
 * Dexie, chrome.storage, and chrome.alarms are all available in any extension
 * page, not just the background worker, so no message round-trip is needed for
 * this.
 *
...
export async function resetAllData(): Promise<void> {
  await Promise.all([clearAllData(), storage.clearAll(), chrome.alarms.clear(ALARM_NAMES.TRAVEL_ARRIVAL)]);
}
