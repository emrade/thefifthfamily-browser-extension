...
export type ExtensionMessage =
  ...
  // Sent by the in-page Arena panel on mount — same bootstrap role as
  // 'street-intel-viewed': arms the page reminder's alarm the first time the
  // player is seen on Arena, rather than running it from install for an
  // account that never touches the page. See
  // background/features/arena/watcher.ts.
  | { type: 'arena-viewed'; timestamp: number }
  // Sent by the in-page Arena panel for the reminder's last-seen state
  // (`ArenaWatchStatus | null`).
