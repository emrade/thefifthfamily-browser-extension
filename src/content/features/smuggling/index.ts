...
import { parseSmugglingV2Panel } from './adapters/smugglingPanelAdapter';
...
export function handleCapturedRequest(req: CapturedRequest) {
  ...
  if (path.endsWith('/actions/travel_proto.php') && req.method === 'POST') {
    ...
    if (message) {
      ...
      // survived the rename unverified) is routine.
      recordParseFailure('smuggling');
    }
    return;
  }
